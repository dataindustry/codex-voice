...
本機優先的 macOS 語音輸入工具。按一次內建全域快捷鍵開始錄音，再按一次提交；Codex Voice 會把中文口述和 IT 英文術語轉成文字，經過本機規則與 Ollama 校正後寫入剪貼簿，並在目前焦點確認是輸入框時自動貼上。

語言版本：[English](README.md) | [简体中文](README.zh-CN.md) | 繁體中文 | [日本語](README.ja.md)

## 適合誰

- 經常在 Codex Desktop、Cursor、VS Code、瀏覽器、聊天工具或任何文字輸入框中，用中文夾雜英文技術詞輸入。
...
- 本機校正：預設使用 Ollama 的 `qwen3.6:35b-a3b`，保守修正中文錯字、技術術語和格式。
...
- 如果預設 qwen 模型沒有安裝，介面會顯示「未安裝 qwen3.6:35b-a3b」；程式不會自動下載大型模型。
...
| 模型 | 推薦程度 | 說明 |
| --- | --- | --- |
| `qwen3.6:35b-a3b` | 預設推薦 | 中文口述校正、IT 術語保留和保守改寫最穩。需要較多記憶體，適合常駐載入。 |
...
以下圖片是穩定路徑的介面截圖說明。之後可用同名真實截圖取代這些 SVG，README 連結不需要改。

### 狀態列主面板

![狀態列主面板截圖說明](docs/assets/screenshots/status-panel.svg)
...
- 權限與設定：麥克風權限、輔助使用權限、內建快捷鍵錄製、清除、恢復預設，以及錄音浮窗開關都在這裡管理。
- 標籤頁：`轉錄模型`、`校正模型`、`輸入裝置` 切換下方卡片區；卡片區高度會跟隨目前頁面內容收緊，不保留其他標籤頁的舊高度。
- 底部摘要：集中顯示目前狀態、轉錄模型、校正模型和輸入裝置，方便確認最後生效的設定。

### 模型卡片

![模型卡片截圖說明](docs/assets/screenshots/model-cards.svg)
...
![內建快捷鍵截圖說明](docs/assets/screenshots/native-hotkey.svg)
...
![退出確認截圖說明](docs/assets/screenshots/quit-unload.svg)
