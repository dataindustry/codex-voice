...
def set_runtime_root(root: str | os.PathLike[str] | None) -> AppPaths:
    global PATHS, ROOT, CONFIG_PATH, STATE_DIR, MODEL_TASK_PATH
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="List currently loaded Ollama models without starting Ollama.",
    )
    parser.add_argument(
...
def ensure_ollama_service(config: dict[str, Any]) -> tuple[bool, str, str]:
    ...
    started, start_error = start_ollama_processes(config)
    if not started:
...
def classify_ollama_model(config: dict[str, Any], model: dict[str, Any]) -> dict[str, Any]:
    ...
    completion_capable = "completion" in capabilities or not capabilities
    correction_candidate = (
        configured_correction_model == name
        or (completion_capable and not transcription_candidate)
    ) and not embedding
...
def list_loaded_ollama_models(config: dict[str, Any]) -> None:
    base_url = ollama_base_url(config)
    try:
        running = ollama_get_json(config, "/api/ps")
        running_models = running.get("models", [])
...
def main() -> int:
    ...
    if args.list_loaded_ollama_models:
        list_loaded_ollama_models(config)
        return 0
