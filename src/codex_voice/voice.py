...
TERMS_PATH = PATHS.terms_path
PROMPT_PATH = PATHS.prompt_path
LOG_PATH = PATHS.log_path
PID_PATH = PATHS.recording_pid_path
INDICATOR_PID_PATH = PATHS.indicator_pid_path
SUBMIT_REQUEST_PATH = PATHS.submit_request_path
STATUS_PATH = PATHS.status_path
LOCK_PATH = PATHS.state_lock_path
PASTE_REQUEST_PATH = PATHS.paste_request_path
PASTE_RESULT_PATH = PATHS.paste_result_path
RUNTIME_PATHS = (
    "/opt/homebrew/bin",
    "/usr/local/bin",
    "/usr/bin",
...
MIN_MANUAL_SUBMIT_AGE_SECONDS = 1.5
...
def set_runtime_root(root: str | os.PathLike[str] | None) -> AppPaths:
    global PATHS, ROOT, BIN_DIR, CONFIG_DIR, RECORDINGS_DIR, TRANSCRIPTS_DIR, LOGS_DIR, STATE_DIR
    global CONFIG_PATH, TERMS_PATH, PROMPT_PATH, LOG_PATH, PID_PATH, INDICATOR_PID_PATH
    global SUBMIT_REQUEST_PATH, STATUS_PATH, LOCK_PATH, PASTE_REQUEST_PATH, PASTE_RESULT_PATH
...
def command_looks_like_codex_voice_worker(command: str) -> bool:
    normalized = command.replace("\\", "/")
    if "--worker" not in normalized or "CodexVoice" not in normalized:
        return False
    return (
        "codex-voice.py" in normalized
        or "codex_voice/voice.py" in normalized
        or "codex_voice.voice" in normalized
    )
...
def pid_looks_like_codex_voice(pid: int) -> bool:
    ...
    if not command:
        return False
    if command_looks_like_codex_voice_worker(command):
        return True
    return pid_file_is_fresh()


def discover_recording_worker_pids() -> list[int]:
    try:
        result = subprocess.run(
            ["ps", "-axo", "pid=,command="],
            capture_output=True,
    ...
    for line in result.stdout.splitlines():
        ...
        if pid == current_pid:
            continue
        if command_looks_like_codex_voice_worker(parts[1]):
            pids.append(pid)
    return sorted(pids)


def recover_recording_pid_from_process_table() -> int | None:
    pids = discover_recording_worker_pids()
    if not pids:
        return None
    pid = pids[-1]
    mark_recording_active(pid)
    return pid
...
def active_recording_pid() -> int | None:
    ...
    if pid is None:
        SUBMIT_REQUEST_PATH.unlink(missing_ok=True)
        return recover_recording_pid_from_process_table()
...
def recording_age_seconds(pid: int) -> float | None:
    try:
        text = PID_PATH.read_text(encoding="utf-8").strip()
        if int(text) != pid:
            return None
...
def request_manual_submit(
    logger: logging.Logger,
    min_recording_age_seconds: float = 0.0,
) -> tuple[int | None, bool]:
    pid = active_recording_pid()
    if pid is None:
        return None, False

    age_seconds = recording_age_seconds(pid)
    if (
        min_recording_age_seconds > 0
        and age_seconds is not None
        and age_seconds < min_recording_age_seconds
    ):
        logger.info(
            "Ignoring manual submit for recording worker %s after %.2fs (< %.2fs).",
            pid,
            age_seconds,
            min_recording_age_seconds,
        )
        return pid, False
...
def load_config(path: Path | None = None) -> dict[str, Any]:
    ...
    merged.update(migrate_config(user_config))
    return merged
...
def record_audio(config: dict[str, Any], logger: logging.Logger) -> Any:
    ...
    try:
        ...
        raise CodexVoiceError(
            ...
            "Codex Voice Agent, Terminal, or Python."
        ) from exc
    finally:
...
def repeated_unit_ratio(text: str) -> tuple[float, str]:
    ...
    for unit_length in range(1, max_unit_length + 1):
        ...
        matches = sum(1 for left, right in zip(compact, repeated, strict=False) if left == right)
...
def paste_to_frontmost_app(
    ...
    if bool(config.get("paste_requires_editable_focus", True)):
        ...
        if focus_status == "unknown":
            logger.info(
                "Could not verify editable focus; left final text in clipboard: %s|%s",
                app_name,
                detail,
            )
            notify_status(config, ACCESSIBILITY_REENABLE_MESSAGE, logger)
        ...
        logger.info(
            "Focused element is not editable; left final text in clipboard: %s|%s|%s",
            focus_status,
            app_name,
            detail,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Codex Voice runtime root. Precedence: --root, CODEX_VOICE_HOME, ~/CodexVoice.",
    )
    parser.add_argument(
        "--toggle",
        action="store_true",
        help="Native hotkey mode: start recording in the background, or submit if already recording.",
    )
    parser.add_argument(
        "--submit-current",
        action="store_true",
